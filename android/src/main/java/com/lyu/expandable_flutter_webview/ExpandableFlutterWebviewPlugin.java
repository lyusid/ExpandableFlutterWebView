...
/**
 * ExpandableFlutterWebviewPlugin
 */
public class ExpandableFlutterWebviewPlugin implements MethodCallHandler, PluginRegistry.ActivityResultListener {
    private Activity activity;
    private WebviewManager webViewManager;
    static MethodChannel channel;
    private static final String CHANNEL_NAME = "expandable_flutter_webview";

    public static void registerWith(PluginRegistry.Registrar registrar) {
        channel = new MethodChannel(registrar.messenger(), CHANNEL_NAME);
        final ExpandableFlutterWebviewPlugin instance = new ExpandableFlutterWebviewPlugin(registrar.activity());
        registrar.addActivityResultListener(instance);
        channel.setMethodCallHandler(instance);
    }

    public static void registerWith(PluginRegistry registrar) {
        PluginRegistry.Registrar registrarFor = registrar.registrarFor("com.lyu.expandable_flutter_webview.ExpandableFlutterWebviewPlugin");
        channel = new MethodChannel(registrarFor.messenger(), CHANNEL_NAME);
        final ExpandableFlutterWebviewPlugin instance = new ExpandableFlutterWebviewPlugin(registrarFor.activity());
        registrarFor.addActivityResultListener(instance);
        channel.setMethodCallHandler(instance);
    }

    private ExpandableFlutterWebviewPlugin(Activity activity) {
    ...
    private void openUrl(MethodCall call, MethodChannel.Result result) {
        ...
        String hostUrl = call.argument("hostUrl");
        if (webViewManager == null || webViewManager.closed == true) {
            Map<String, Number> rc = call.argument("rect");
            FrameLayout.LayoutParams params;
            int width = 0;
            if (rc != null) {
                width = rc.get("width").intValue();
            }
            webViewManager = new WebviewManager(activity, width);
        }
